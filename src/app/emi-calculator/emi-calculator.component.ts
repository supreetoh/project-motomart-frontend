...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, NgIf], // Include NgIf in imports
...
export class EmiCalculatorComponent {
  ...
  calculateEMI(): void {
    console.log('calculateEMI method called'); // Debugging line
    ...
    if (this.loanAmount > 0 && this.interestRate > 0 && this.loanTenure > 0) {
      ...
      this.emi = parseFloat(this.emi.toFixed(2)); // Round to 2 decimal places

      console.log(`EMI Calculated: ${this.emi}`); // Debugging line
    } else {
      this.emi = null;
      console.log('Invalid input values'); // Debugging line
    }
  }
}
*/
